...
int main () {
    pBuffer = malloc ( 53 ); // aloca o pAgenda
    * (int * ) ( pBuffer + 1 ) = 0; // int entrada = 0
    *( int * ) ( pBuffer + 5 ) = 0; // int numeroDePessoas = 0
    * ( void ** ) ( pBuffer + 13 ) = NULL; // o inicio da agenda é NULL
    * ( void ** ) ( pBuffer + 37 ) = NULL; // o início da fila auxiliar é NULL
...
void RemoverPessoa ( ) {
    ...
    if ( busca == NULL ) {
        ...
        } else if ( busca == * ( void ** ) ( pBuffer + 21 ) ) { // se a pessoa for a ultima da lista
...
void *BuscarPessoa ( ) {
    ...
    * ( char * ) ( pBuffer + 29 + * ( int * ) ( pBuffer + 9 ) ) = '\0';
    ...
    // ponteiros auxiliares para manipulação das filas
    void *atual = * ( void ** ) ( pBuffer + 13 ); // percorre a agenda
    void *filaAuxInicio = NULL; // ponteiro para o início da fila auxiliar
    

    // busca a pessoa e, se não encontrar, transfere para a fila auxiliar
    while ( atual != NULL && strcmp ( ( char * ) atual, ( char * ) ( pBuffer + 29 ) ) != 0 ) {

        // adiciona no início da fila auxiliar
        void *novoNodo = malloc(60);

        // copia dados para o novo nodo
        strcpy ( ( char * ) novoNodo, ( char * ) atual );
        * ( int * ) ( novoNodo + 20 ) = * ( int * ) ( atual + 20 );
        strcpy ( ( char * ) ( novoNodo + 24 ), ( char * ) ( atual + 24 ) );

        // coloca no inicio da fila auxiliar
        ...
        filaAuxInicio = novoNodo; // passa os dados do novo nodo pro nodo inicial da fila auxiliar
        atual = * ( void ** ) ( atual + 52 ); // atual vai para a próxima pessoa na fila principal

    }

    // atualiza o início da fila principal
    * ( void ** ) ( pBuffer + 13 ) = atual;
    if ( atual != NULL ) { 
        * ( void ** ) ( atual + 44 ) = NULL; // se atual for válido, o anterior a ele vira NULL
    } else {
        * ( void ** ) ( pBuffer + 21 ) = NULL; // se atual não for válido, fim da fila principal vira NULL
    }

    // insere os dados da fila auxiliar de volta para a fila principal
    while (filaAuxInicio != NULL) {

        void *proximaAux = * ( void ** ) ( filaAuxInicio + 52 ); // próximo na fila auxiliar

        // insere no início da lista principal
        if ( * ( void ** ) ( pBuffer + 13 ) == NULL) { // primeiro nodo a ser inserido:

            * ( void ** ) ( pBuffer + 13 ) = filaAuxInicio; // atualiza o inicio da fila principal
            * ( void ** ) ( pBuffer + 21 ) = filaAuxInicio; // atualiza o inicio da fila auxiliar
            * ( void ** ) ( filaAuxInicio + 44 ) = NULL; //filaAuxInicio->ant = NULL
            * ( void ** ) ( filaAuxInicio + 52 ) = NULL; //filaAuxInicio->prox = NULL
        
        } else { // se não for o primeiro:

            * ( void ** ) ( filaAuxInicio + 52 ) = * ( void ** ) ( pBuffer + 13 ); //filaAuxInicio->prox = inicio da fila principal
            * ( void ** ) ( * ( void ** ) ( pBuffer + 13 ) + 44 ) = filaAuxInicio; // filaAuxInicio vira ant do inicio da fila principal
            * ( void ** ) ( filaAuxInicio + 44 ) = NULL; //filaAuxInicio->ant = NULL
            * ( void ** ) ( pBuffer + 13 ) = filaAuxInicio; // filaAuxInicio vira inicio da fila principal
        }

        filaAuxInicio = proximaAux; // próximo nodo
