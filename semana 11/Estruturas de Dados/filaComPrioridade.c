...
bool Push(SFila *fila, int num){
    ...
    if(num<= fila->pFirst->info){
        nodo->next = fila->pFirst;
        fila->pFirst = nodo;
    }

    STipoFila *posicaoAtual = fila->pFirst;
    STipoFila *posicaoAnterior = NULL;

    // se eu tiver um numero 4, ele entra depois do numero 3 e antes do 5 e depois dos 4 q ja estiverem
    // quando encontrar um 5, devo parar e pegar o anterior a ele

    while (posicaoAtual != NULL && posicaoAtual->info < num) {
        posicaoAnterior = posicaoAtual;
        posicaoAtual = posicaoAtual->next;
    }

    posicaoAnterior->next = nodo;
    nodo->next = posicaoAtual;
